...
function loadFunction($scope, structureService, $location, FirebaseAuth){
  ...
  var auth = FirebaseAuth.init($scope.static_loginregister.modulescope.firebaseSrc);
  $scope.send = function() {
    var loginData = {
       email: $scope.static_loginregister.email,
       password: $scope.static_loginregister.pwd
    };
    auth.$authWithPassword(loginData).then(function(userData) {
  ...
  function createuser(loginData) {
    auth.$createUser(loginData).then(function(userData) {
